...
The contract also contains an enum of STATUS(constants) which are 
        ...
        SOHM,
        OHMDEBTOR
    Structs in the contract also collect the following data  STATUS managing;
        address toPermit;
        address calculator;
        uint256 timelockEnd;
        bool nullify;
...
The Mint Function:
Its a RewardManager only function which can be initialized by only the Rewards Manager.
